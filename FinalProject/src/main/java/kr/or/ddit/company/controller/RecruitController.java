...
//--------------------------------채용공고 리스트----------------------------------------------	
	...
	// 지역 list + 페이징 data처리
	//Map은 @RequestParam으로 받는다 (modelX)
	...
	public String regionListData(@RequestParam Map<String, Object> variousCondition,
		...
		//{tregCode=JN, regCode=, rcrtTitle=, page=}
...
//		 GET: /board/23/edit, PUT /board/23 get은 양식 제공, PUT 수정 등록 
//	RCRT_00063
...
//			recruitVo.setRcrtKeywordnm("개발");  //  하드코딩
			...
			case OK:
				...
				// 우선 임시로 채용공고 채용공고 지역 리스트로..
				viewName = "users/recruit/recruitRegionList";
				break;
...
//	// 채용공고 업데이트시 전형절차 추가 버튼 누를때마다 insert 잘 모르겠음...
//	@PostMapping("/recruit/{reNo}/rproNew")
//	@ResponseBody
//	public String rproInsert(@PathVariable("reNo")String reNo, 
//						 @RequestBody RProcedureVO vo
//						) {
//
//		vo.setRcrtNo(reNo); //채용공고번호
//		String msg = "NG";
//		ServiceResult result = rproService.createProcedure(vo);
//		
//		if(result == ServiceResult.OK) {
//			msg = "OK";
//		}
...
//	차트
//	@GetMapping("/recruit/{reNo}/chartMale")
//	@ResponseBody
//	public RecruitVO selectchartMale(@PathVariable String reNo, @RequestBody RecruitVO vo) {
//		int result = reService.selectchartMale(reNo);
//		return "";
//	}
}
