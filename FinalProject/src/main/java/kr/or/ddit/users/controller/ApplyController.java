...
public class ApplyController {
	...
	private Resource recruitFiles;
	
	@ResponseBody
	@GetMapping("/apply/resumeList")
	public List<ResumeFileInfoVO> resumeList(
			@SessionAttribute("authId")String usersId
			){
		
//		String id="az001"; //하드코딩
		List<ResumeFileInfoVO> list = service.retrieveResumeList(usersId);
		return list;
	}
...
//		applyVo.setResattNo(applyVo.getResattNo()); //  어떻게 가져와????????????????
	...
	*/
		...
		public String resumeImageUpload(ResumeFileInfoVO sowonDTO,HttpServletRequest request) throws Exception {

			String saveFolder = "d:/finalprojectDoc/uploadFiles/resume/";  // 확인하기 좋게 일단 하드코딩 appinfo!!!!!!!
			
			MultipartFile upload = sowonDTO.getUpload();
			...
			if(!upload.isEmpty()) {
				...
				vo.setResumeTitle(sowonDTO.getSwName()); //일단 하드코딩!!!!!!!!! 이력서 제목
		...
		public Map<String,Object> resumeDetail(@PathVariable String resumeNo) {
			...
			ResumeFileInfoVO resume = resumeService.retrieveResume(resumeNo);
